                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('produk.update', $produk->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="4" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
                            <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="harga_beli" :value="__('Harga Beli')" />
                            <x-text-input id="harga_beli" class="block mt-1 w-full" type="number" step="0.01" name="harga_beli" :value="old('harga_beli', $produk->harga_beli)" required />
                            <x-input-error :messages="$errors->get('harga_beli')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="harga_jual" :value="__('Harga Jual')" />
                            <x-text-input id="harga_jual" class="block mt-1 w-full" type="number" step="0.01" name="harga_jual" :value="old('harga_jual', $produk->harga_jual)" required />
                        ...
                        <div class="mb-4">
                            ...
                            @if ($produk->gambar_produk)
                                <div class="mb-2">
                                    <img src="{{ asset($produk->gambar_produk) }}" alt="{{ $produk->nama_produk }}" class="w-24 h-24 object-cover rounded">
                            ...
                            <p class="mt-1 text-sm text-gray-500" id="file_input_help">Kosongkan jika tidak ingin mengubah gambar. JPEG, PNG, JPG (MAX. 2MB).</p>
                        ...
                        <div class="mb-4 flex items-center">
                            <input type="checkbox" id="aktif" name="aktif" value="1" {{ old('aktif', $produk->aktif) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
